...
- **Job boards:** "The framework includes tools for the German/DACH market: Arbeitnow (free, tech/remote-focused), plus Stepstone and Xing via Apify (need an APIFY_TOKEN). Are these the right ones for you, or do you use other sites?" Note: if the user is outside Germany/DACH, acknowledge that the built-in CLI tools target German boards and suggest they can add their own portal integrations (Arbeitnow is the simplest pattern to copy for a free-API board) or rely on LinkedIn/Google site-searches.
